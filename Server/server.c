...
void response_static_file(int client_socket_fd, char filename[BUFFER_SIZE], off_t size_file);
 ...
 */
void cerror(int client_socket_fd, char *cause, char *errno,char *shortmsg, char *longmsg) {
...
void execute_server(char *work_dir, char *server_port) {
    ...
    if (server_port) {
        ...
        port_number = atoi(DEFAULT_SERVER_PORT);
    }
    ...
    char filename[BUFFER_SIZE];/* path derived from uri */
    int is_static;         /* static request? */
    char cgi_args[BUFFER_SIZE]; /* cgi argument list */
    ...
    while (1) {
        ...
        recv(client_socket_fd, buf, BUFFER_SIZE, 0);
...
void response_static_file(int client_socket_fd, char filename[BUFFER_SIZE], off_t size_file) {
    ...
    ssize_t rbytes = 0;
    while (1) {
        rbytes = pread(fd, buf, BUFFER_SIZE, offset);
        if (rbytes <= 0) {
            break;
        }
